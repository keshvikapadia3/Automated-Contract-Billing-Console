...
const InvoicePage = () => {
  ...
  const grandTotal = contracts.reduce((sum, c) => sum + getTotalValue(c.id), 0);
      ...
      <Box sx={{ display: "flex", gap: 3, mb: 4 }}>
        ...
        <Card sx={{ flex: 1, borderTop: "4px solid #0077b6", borderRadius: 2 }}>
          <CardContent>
            ...
            <Typography variant="h4" sx={{ fontWeight: "bold", color: "#0077b6" }}>{points.length}</Typography>
          </CardContent>
        </Card>
        <Card sx={{ flex: 1, borderTop: "4px solid #00b4d8", borderRadius: 2 }}>
          <CardContent>
            <Typography variant="body2" color="text.secondary">Grand Total Value</Typography>
            <Typography variant="h4" sx={{ fontWeight: "bold", color: "#00b4d8" }}>{grandTotal}</Typography>
          </CardContent>
        </Card>
      </Box>
